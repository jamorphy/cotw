...
def compile_video(work_dir, discord_channel_id):
    metadata_path = os.path.join(work_dir, "clips_metadata.json")
    with open(metadata_path, 'r') as f:
        clips_metadata = json.load(f)
...
if __name__ == "__main__":
    ...
    min_views, min_runtime, discord_channel_id = get_editor_info(games, args.game_name)
    ...
    single_turn.initiate_chat(
        ...
        message=f"Let's check if there's enough runtime to compile a video, work_dir: {working_folder}, target_runtime: {min_runtime}, discord_channel_id: {discord_channel_id}",
    )